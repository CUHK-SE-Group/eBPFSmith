...
func (vr *ValidationResult) GetDidCollectCoverage() bool {
	return vr.DidCollectCoverage
}

func (vr *ValidationResult) GetCoverageAddress() []uint64 {
	return vr.CoverageAddress
}
...
type Metrics struct {
	...
	validationResultQueue []*ValidationResult
...
func (mu *Metrics) enqueueValidationResult(vr *ValidationResult) {
...
func (mu *Metrics) dequeueValidationResult() *ValidationResult {
	mu.validationMutex.Lock()
	defer mu.validationMutex.Unlock()
	...
	if len(mu.validationResultQueue) == 1 {
		mu.validationResultQueue = []*ValidationResult{}
	} else {
...
func (mu *Metrics) RecordVerificationResults(vr *ValidationResult) {
	if vr.GetIsValid() {
...
func NewMetricsUnit(threshold int, kcovSize uint64, vmLinuxPath, sourceFilesPath, metricsServerAddr string, metricsServerPort uint16, cm *CoverageManagerImpl) *Metrics {
